...
const FAQBlock = ({photoUrl, title} : {photoUrl:string, title:string}) => {
    ...
    return(
        <div ref={faqRef} className={`flex flex-col h-fit gap-2 font-Acrom_Light opacity-0 ${isBlockVisible ? 'animateOpacityItem' : ''}`}>
            <ImageSeeMore linkSeeMore="/" photoUrl={photoUrl}></ImageSeeMore>
